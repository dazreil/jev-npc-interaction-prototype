...
Piper is the preferred speech engine in every browser. It is a neural voice, so Arthur sounds like a recorded person rather than a rule-based synthesiser. A module Web Worker loads the voice model and performs inference away from the interface thread. Both engines emit 22.05 kHz mono PCM, which the game converts into a Web Audio buffer and passes through the same narrow-band intercom filter and compression used by the terminal effects. Arthur's persistent character profile and the authored performance still control pauses and the intercom filter. Rate and pitch remain available to the fallback engines, while Piper keeps the neural model's native delivery.

Each line is synthesised one sentence at a time and scheduled gaplessly. Arthur starts speaking after the first sentence rather than after the whole line, which cuts the wait before a long reply by roughly two thirds. Piper keeps the voice model's native speed and pitch; applying the older performance pitch values through buffer resampling made it sound artificially slowed down. An intercom key-up click covers the remainder.
...
This runs the automated tests, dialogue lint, and the offline release audit. The audit checks the portrait manifest, asset-size budget, lazy-loading boundary, credits and diagnostics hooks, reduced-motion rule, server MIME types, and deployment files. Browser observations are recorded in [PHASE15_RELEASE_CHECK.md](PHASE15_RELEASE_CHECK.md); open the game at [http://localhost:5173](http://localhost:5173), never from a `file://` URL.
...
- Acknowledge a contradiction or hostile moment, repair the conversation, then rebuild a credible spoken case.
...
Arthur has an estimated IQ of 95. He is practical and ordinarily capable rather than analytical, uses everyday language, and assumes visitors at a staffed industrial gate already know the basic procedure. The selected character profile changes his starting mood, personality values, trust threshold, phrasing, speech rate, pitch, and pre-delay. Jev receives the same cognitive note, profile, and fixed intercom layout in its structured decision context. Claims record a purpose, contradictions and admitted lies sharply increase suspicion, and unrepaired serious memories can remove `ALLOW_ENTRY` from the actions sent to the provider. Arthur raises each suspicious incident once; `BECOME_SUSPICIOUS` then remains unavailable until the player clarifies or apologizes through `REPAIR_CONVERSATION`, preventing repeated accusation loops. Arthur also asks for the player's purpose once per conversational attempt; if the player stays vague, he holds the boundary instead of asking a reworded version of the same question. A sincere apology or clarification lowers tension and marks the earlier damage as repaired so the player can rebuild a credible case. Message effort also nudges mood: terse replies add a small amount of irritation, while considered explanations can slightly improve trust. A first weapon threat selects `DEESCALATE_THREAT`: Arthur reminds the player that he is in the guard tower and they are outside the locked car-park gate, then calmly asks them to lower the weapon and explain what they need.
...
Selecting Jev sends the same decision context to the local server, where TypeSafe evaluates an action `choice` and a parallel `noul` judgment asking whether the player's spoken case is credible. Jev's selected action, probability distribution, confidence, credibility probability, model, and token usage are inspectable in the raw response view. `ALLOW_ENTRY` requires at least `0.50` action confidence and `0.55` credibility; otherwise Arthur asks for a more convincing explanation. A vague claim such as “I have them” is weak, but a consistent story, specific operational details, urgency, respectful persistence, or earned trust can persuade Arthur without an inventory item. State changes and memory records remain deterministic code-side effects.
