...
Arthur has an estimated IQ of 95. He is practical and ordinarily capable rather than analytical, uses everyday language, and assumes visitors at a staffed industrial gate already know the basic procedure. The selected character profile changes his starting mood, personality values, trust threshold, phrasing, speech rate, pitch, and pre-delay. Jev receives the same cognitive note, profile, and fixed intercom layout in its structured decision context. Claims record a purpose, contradictions and admitted lies sharply increase suspicion, and unrepaired serious memories can remove `ALLOW_ENTRY` from the actions sent to the provider. Arthur raises each suspicious incident once; `BECOME_SUSPICIOUS` then remains unavailable until the player clarifies, apologizes, or supplies evidence through `REPAIR_CONVERSATION`, preventing repeated accusation loops. Arthur also asks for the player's purpose once per conversational attempt; if the player stays vague, he holds the boundary instead of asking a reworded version of the same question. A sincere apology or clarification lowers tension and marks the earlier damage as repaired so later credible proof can reopen car-park access. Message effort also nudges mood: terse replies add a small amount of irritation, while considered explanations can slightly improve trust. A first weapon threat selects `DEESCALATE_THREAT`: Arthur reminds the player that he is in the guard tower and they are outside the locked car-park gate, then calmly asks them to lower the weapon and explain what they need.
