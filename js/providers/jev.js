...
export const ACTION_CRITERIA = Object.freeze({
  ...
  DEESCALATE_THREAT:
    "Use for the first explicit gun, firearm, weapon, or shooting threat. Arthur is inside the locked gatehouse and sees the player outside through the intercom camera, so the weapon cannot directly force him to open the entrance. Calmly state that physical separation, invite the player to lower the weapon, and ask what they need. Do not act as though Arthur and the player are face to face, threaten police, or provoke the player.",
  THREATEN_PLAYER:
    "Warn that security will be called when the outside player says they will attack the locked entrance, force entry, or continues a credible threat after Arthur has already tried to de-escalate over the intercom. Arthur remains inside the secure gatehouse. Do not use this for a first weapon threat when DEESCALATE_THREAT is available.",
  SHOW_SYMPATHY:
...
export function buildJevRequest(context, availableActions) {
  ...
  return {
    state: {
      npc: {
        id: context.npc?.id,
        role: "Night security guard inside a locked gatehouse, speaking to an outside visitor through a camera intercom",
