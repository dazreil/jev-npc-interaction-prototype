...
- eSpeak NG 0.3.5 now runs locally through its Emscripten WebAssembly build and supplies PCM audio to a filtered Web Audio playback chain. A module Web Worker keeps runtime initialization and synthesis off the interface thread. Its 24 MB voice data is installed locally and loaded only on first use; Web Speech and timed captions remain automatic fallbacks.
